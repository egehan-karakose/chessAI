    ...
    for i in range(8):
        for j in range(8):
            if get_piece(board.fen(), (i * 8) + j) != "blank":
                buttons[i*8 +
                        j].setIcon(QIcon(get_piece(board.fen(), (i * 8) + j)))
            else:
...
class App(QWidget):
    ...
    def initUI(self):
        ...
        if self.board.is_checkmate():
            # Indicates someone win by checkmate
            self.label.setText("CheckMate")
            game_over(self.buttons)
        elif self.board.is_game_over():
            # Indicates that game is over due to non-checkmate scenario (maybe draw)
        ...
        if self.first_position == "none":
            ...
            if from_position_to_chess_move(self.first_position) != from_position_to_chess_move(self.second_position):
                ...
                if self.board.is_legal(chess.Move.from_uci(algebraic_move)) or self.board.is_legal(chess.Move.from_uci(algebraic_move+"q")):
                    # You can only promote to queen but it is the best anyway :)
                    if self.board.is_legal(chess.Move.from_uci(algebraic_move+"q")):
                        algebraic_move += "q"
                    ...
                    if self.board.is_checkmate():
                        ...
                        # somehow label couln't get updated in update_board
                        if self.board.fen().split(" ")[1] == "w":
                            self.label.setText("Turn: White")
                        else:
                            self.label.setText("Turn: Black")

                    # get AI move, but we give AI copy of the board so it can play with it.
                    ai_move = ai_play(self.board.copy())
            ...
            if (((self.first_position // 8) % 2) + (self.first_position % 8)) % 2 == 0:
                self.buttons[self.first_position].setStyleSheet(
